...
public class SimplificationsFromExamplesAction  extends Action implements IAction{
    ...
    /**
     * root attribute for all templateId elements in NHS CDAs
     */
    public static String NHS_TEMPLATE_ROOT = "2.16.840.1.113883.2.1.3.2.4.18.2";
	
	public SimplificationsFromExamplesAction()
	{
		super("Define Simple Message from Example Messages");
	}
	 ...
	 */
	protected MappedStructure mappedStructure()
	{
		return (MappedStructure)mapperEditor.getEditingDomain().getResourceSet().getResources().get(0).getContents().get(0);
	}
	...
	private Element getExampleMessageAndWrapIn(String dialogTitle, boolean findWrapperClass) throws MapperException
	{
		...
		String[] exts = {"*.xml"}; 
		String path = FileUtil.getFilePathFromUser(mapperEditor,exts,dialogTitle,false);
	...
	private boolean annotateModel(LabelledEClass theClass, Element el1,Element el2) throws MapperException
	{
		...
		for (int a = 0; a < el1.getAttributes().getLength(); a++)
		{
			...
			{
				if (markAttributeUsedOrFixed(theClass,att.getName(), val1, val2,universalFixedValue(val1))) hasUsedAttributes = true;
			}
		}
		...
		if (XMLUtil.childElements(el1).size() == 0)
		{
			...
			if ((!val1.equals(val2))|universalFixedValue(val1))
			{
				...
				if (markAttributeUsedOrFixed(theClass,"textContent", val1, val2,universalFixedValue(val1)))  hasUsedAttributes = true;				
			}
		}
		...
		{
			addTextContentAttribute(theClass.eClass());
			if (markAttributeUsedOrFixed(theClass,"textContent", "a", "a#",false))  hasUsedAttributes = true;							
		}
		...
		if (hasUsedAttributes) mark_CS_CV_CD_II_BL_Attributes(theClass);
		
		return hasUsedAttributes;
	}
	
	/**
	 * Only called for NHS MIFs.
	 ...
	 * lead only to an II or CV or BL data type object. Mark the 'extension' or 'code' attribute
	 ...
	 * @param theClass
	 */
	public static void mark_CS_CV_CD_II_BL_Attributes(LabelledEClass theClass) throws MapperException
	{
		for (Iterator<EReference> ir = theClass.eClass().getEAllReferences().iterator(); ir.hasNext();)
		{
			...
			if (fixedVal != null)
			{
				...
				if (child != null)
				{
					...
					if (nextCName.equals("II")) 
					{
						markAttributeUsedOrFixed(child,"extension",val1,val2,false);
						// for NHS templateId elements, add a fixed root value
						if (ref.getName().equals("templateId"))markAttributeUsedOrFixed(child,"root",NHS_TEMPLATE_ROOT,NHS_TEMPLATE_ROOT + "#f",false);
					}
					else if (nextCName.equals("CV")) markAttributeUsedOrFixed(child,"code",val1,val2,false);
					else if (nextCName.equals("CD")) markAttributeUsedOrFixed(child,"code",val1,val2,false);
					else if (nextCName.equals("BL")) markAttributeUsedOrFixed(child,"value",val1,val2,false);
					else if (nextCName.equals("CS")) markAttributeUsedOrFixed(child,"code",val1,val2,false);
					...
					// flatten the association if it has maximum multiplicity = 1
					if (ref.getUpperBound() == 1)
					{
						ITSAssociation itsa = FeatureView.getITSAssociation(ref, theClass.getPath());
						itsa.setCollapsed(true);
	 ...
	 * @param aClass
	 */
	public static void addTextContentAttribute(EClass aClass)
	{
	 ...
	 * @throws MapperException
	 */
	public static boolean markAttributeUsedOrFixed(LabelledEClass theClass,String attName, String val1, String val2, boolean universalFixedValue) 
	throws MapperException
	{
		boolean used = false;
		...
		if ((newName.equals("f"))|(newName.equals("fa"))|(universalFixedValue))
		{
	...
	throws MapperException
	{
		...
		if (oMap != null)
		{
			...
			for (Iterator<EAttribute> it = theClass.eClass().getEAllAttributes().iterator(); it.hasNext();)
			{
				...
				String fixedVal = theClass.getAnnotatedFixedValue(attName);
