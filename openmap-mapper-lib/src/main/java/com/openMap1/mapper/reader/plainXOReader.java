...
*  
*  This is a copy of MDLXOReader, made on 12/11/2009 before I started making the 
*  changes for a mapping debugger.
*
*  @author  Robert Worden
*  @version 2.06
*/
public class plainXOReader extends MDLBase implements XOReader
{

	private boolean dTrace = false; // used to switch on detailed tracing of association retrieval
  ...
  //--------------------------------------------------------------------------------------
     ...
     */
    public plainXOReader(org.w3c.dom.Element XMLFileRoot, MappedStructure ms, messageChannel mChan)  throws MapperException
    {
    	super(ms,mChan);
     ...
     */
    public plainXOReader(org.w3c.dom.Element XMLFileRoot, MappedStructure ms, EPackage classModel, messageChannel mChan)  throws MapperException
    {
    ...
    public String readerIdentifier()
     ...
     public XOReader getImportedReader(ImportMappingSet ims) throws MapperException
     {
 		...
 		if (reader == null)
 		{
 			...
 			reader = impMS.getXOReader(theXMLFileRoot, null, mChan());
 			
 			// make the new reader share in the pool of readers, and the pool of MDLBases
			if (reader instanceof plainXOReader)
			{
				plainXOReader mReader = (plainXOReader)reader;
     ...
     public XOReader readerRepresentingObject(String qualifiedClassName, Hashtable<String,String> readersUsed)
     {
     	...
     	else
     	{
         	for (Iterator<EObject> it = ModelUtil.getEObjectsUnder(ms(), MapperPackage.Literals.IMPORT_MAPPING_SET).iterator();it.hasNext();) try
         	{
         		...
         		if (xReader instanceof plainXOReader)
         		{
         			plainXOReader reader = (plainXOReader)xReader;
    ...
    */
    public Vector<objectToken> getAllObjectTokens(String className) throws MapperException
    {
        ...
        */
        v = getObjectReps(className);
        addImportedObjectReps(v, className);
    	timer.stop(Timer.GET_OBJECTS);
        return v;
    }
    ...
    private Vector<objectToken> theObjectReps(objectMapping om) throws MapperException
    {
        ...
        try
        {
            ...
            fNodes = filterNodeVector(unfiltered,om.whenConditions(),theXMLFileRoot,false);
    ...
    */
    ...
    {
        Vector<Node> unfiltered,whenFiltered,filtered;
        Node propertyNode;
        String res = null;
        Vector<propertyMapping> pms  = namedPropertyMappings(oRep.className(),oRep.subset(),propertyName);
        int mappings = pms.size();
          ...
          else // no fixed value; find it from the XML.
          {
            ...
            for (int i = 0; i < mappings; i++) if (res.equals(""))
            {
              ...
              try
              {
                ...
                // LHS of filter conditions is got from property nodes, not object node
                whenFiltered = filterNodeVector(unfiltered,pm.whenConditions(),oRep.objNode(),false);
                filtered = filterNodeVector(whenFiltered,pm.linkConditions(),oRep.objNode(),false);
        ...
        String otherClass, int thisEnd, String otherRole) throws MapperException
    {
        ...
        if (otherEClass == null) throw new MapperException("Cannot find other end class '" + otherClass + "'");
...
private Vector<objectRep> followAssocXPaths(objectRep oRep,AssociationMapping am,
    ...
    throws MDLReadException
    {
        ...
        Vector<Node> a2; // link-filtered list of association nodes got from start object node
        Vector<Node> a3; // further when-filtered list of association nodes got from start object node
        ...
        // use boolean dTrace to get selective tracing of this method
        if (dTrace) mChan().message("actualGetAssocObjects: " + oRep.className() + "'" + oRep.subset() + "' "
              + am.fullName() + " " + otherEndCSet.stringForm() + " " + oneOrTwo);
        ...
        try
        {
            ...
            if (dTrace) mChan().message("a1 " + a1.size());
            /* apply any link conditions on the path into the association node,
            to get a filtered list of association nodes */
            a2 = filterNodeVector(a1,thisEnd.linkConditions(),oRep.objNode(),false);
            if (dTrace) mChan().message("a2 " + a2.size());
            /* apply any 'when' conditions on the association node,
            to get a fully filtered list of association nodes */
            a3 = filterNodeVector(a2,am.whenConditions(),oRep.objNode(),false);
            if (dTrace) mChan().message("a3 " + a3.size());
            ...
            for (k = 0; k < a3.size(); k++)
            {
                 ...
                 if (dTrace) mChan().message("b1 " + b1.size());
                 // filter end object nodes with link conditions on the second link
                 b2 = filterNodeVector(b1,otherEnd.linkConditions(),assocNode,true);
                 if (dTrace) mChan().message("b2 " + b2.size());
                 ...
                 b3 = filterNodeVector(b2,endMap.whenConditions(),assocNode,false);
    ...
    */
    protected Vector<Node> filterNodeVector(Vector<Node> a, Vector<?> conditions, Node startNode, boolean startIsLHS)
    throws MapperException
    {
  ...
  //-----------------------------------------------------------------------------------------------
    ...
    private void makeNodeIndex() throws MapperException
    {
    	...
    	for (Enumeration<String> en = pathTable.keys();en.hasMoreElements();) try
    	{
    		...
    		if (writeSummary) System.out.println("Absolute: " + absPath);
			...
			for (Iterator<String> ip = relPaths.iterator(); ip.hasNext();)
			{
				...
				if (writeSummary) System.out.println(relPath + ": " + relPathIndex.size() + ", " + nodesFound);
			}
